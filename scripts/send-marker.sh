...
#
# Usage: scripts/send-marker.sh --type <type> --message <message>
#
# Examples:
#   scripts/send-marker.sh --type deploy --message "deploy cart abc1234"
#   scripts/send-marker.sh --type feature-flag --message "flag cartservice.add-db-call → 50%"
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "https://api.honeycomb.io/1/markers/__all__" \
